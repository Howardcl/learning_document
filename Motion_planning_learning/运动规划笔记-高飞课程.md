...
这个问题就叫做二次优化问题-QP问题。这就是一种非常标准的凸优化问题。代表一定可以求解。
...
数学上的定义：如果在凸函数上去两个点，再在这两个点的中间取一个点，那么中间点的凸函数值是小于这两个点的凸函数值加权平均的。

凸函数为什么重要：如果一个函数是凸的，那么从任何一个起点开始，（只要是处处可导的），那一定可以按梯度方向下降，前进到最低点，如果梯度上升了，此时得到最优值。绝大多数函数其实并不是凸函数，存在很多局部极小值。用基于梯度的方法解出来的解可能是局部极小值。那凸函数的话呢，就保证了解出来的值都是全局极小值。
...
凸优化问题有很多理论在其中，有的问题看上去不是凸的，但可以经过一些数学变换，变成一个等价的凸优化问题，从而求解它的最优解。
...
所以首先要构造的是映射矩阵，把这个问题里，所有待求解的变量P，通过映射矩阵映射成在每一段所有两端端点上的各阶导数。即$M_jP_j=d_j$。
